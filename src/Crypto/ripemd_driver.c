...
/******************************************************
 * Function: ripemd-block
 * Description: computes a message digest for a memory
 * block. Assumes the out array is of the proper size
 * ****************************************************/
void ripemd_block(byte * inblock, int len, byte *out)
{
  RIPEMD_CTX ripe_context;
  ripemd_init (&ripe_context);
  ripemd_update (&ripe_context, inblock, len);
  ripemd_final (&ripe_context);
  ripemd_digest(&ripe_context, out);
  MemSet(&ripe_context, sizeof(RIPEMD_CTX), 0); 
}


/******************************************************
 * Function: ripemd-string
 * Description: computes a message digest for a string.
 * Assumes out array is of the proper size.
 * ****************************************************/
void ripemd_string (char *inString, byte *out)
{
  unsigned int len = StrLen(inString);
  ripemd_block(inString, len, out);
}
