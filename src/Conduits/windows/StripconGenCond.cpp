...
//     Function:        DllMain()
//
//     Description:    main entry point to the stripcon component
//
...
ExportFunc long OpenConduit(PROGRESSFN pFn, CSyncProperties& rProps)
{
    long retval = -1;
    if (pFn) {
		if(rProps.m_SyncType != eHHtoPC) {
			LogAddFormattedEntry(slText,TRUE,"Conduit being opened");
			return SYNCERR_READ_ONLY;
		}

		/* Do the sync magic */
		retval=DoSync(rProps.m_SyncType, rProps.m_PathName);
    }
    return(retval);
}
